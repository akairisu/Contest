...
for i in range(0, LANE_NUM):
    ...
    for j in range(0, Carnum):
        ...
        if TYPE == AUTO:
            car = ellipsoid(TYPE = TYPE, STAT = CONSTANT, size = vec(CAR_LEN, CAR_WIDTH, 1 ), pos = vec(frontpos - AUTODIS - random.random() * CAR_LEN, 10 - 3.65 * i, 0), v = vec(Initspd, 0, 0))
        else:
            car = box(TYPE = TYPE, STAT = CONSTANT, length = CAR_LEN, width = 1, height = CAR_WIDTH, pos = vec(frontpos - SAFEDIS - random.random() * CAR_LEN, 10 - 3.65 * i, 0), v = vec(Initspd, 0, 0))
...
carindex = []
for i in range(0, LANE_NUM):
    car = box(length = CAR_LEN, width = 1, height = 2, pos = vec(Len, 12 - 3.65 * i, 0), v = vec(Initspd, 0, 0), color = color.white)
    carindex.append(car)
    ...
    for i in range(0, LANE_NUM):
	    ...
	    for j in range(firstcar[i], len(car_list[i])):
	        ...
	        if accident == 3:
	            if car_list[i][j].TYPE == ACCIDENT: 
	                car_list[i][j].rotate(axis = vec(0, 0, -1), angle = 0.005)
	    ...
	    if nexttype[i] == AUTO and car_list[i][newcarindex[i] - 1].pos.x > -Len + AUTODIS + random.random() * AUTODIS:
	    	if accident == 1:
		        ...
		        car = ellipsoid(TYPE = ACCIDENT, STAT = frontstat, size = vec(CAR_LEN, CAR_WIDTH, 1), pos = vec(-Len, 10 - 3.65 * i, 0), v = vec(frontv, 0, 0), color = color.orange)
		        car_list[i].append(car)
		        Carnum = Carnum + 1
		        ...
		        frontcolor = car_list[i][newcarindex[i] - 1].color
		        car = ellipsoid(TYPE = nexttype[i], STAT = frontstat, size = vec(CAR_LEN, CAR_WIDTH, 1), pos = vec(-Len, 10 - 3.65 * i, 0), v = vec(frontv, 0, 0), color = frontcolor)
		        car_list[i].append(car)
		        Carnum = Carnum + 1
		        newcarindex[i] = newcarindex[i] + 1
		        if random.random() * 100 > Percent:
	    ...
	    if nexttype[i] == HUMAN and car_list[i][newcarindex[i] - 1].pos.x > -Len + SAFEDIS + random.random() * SAFEDIS:
	        ...
	        frontstat = car_list[i][newcarindex[i] - 1].STAT
	        frontcolor = car_list[i][newcarindex[i] - 1].color
	        car = box(TYPE = nexttype[i], STAT = frontstat, length = CAR_LEN, width = 1, height = CAR_WIDTH, pos = vec(-Len, 10 - 3.65 * i, 0), v = vec(frontv, 0, 0), color = frontcolor)
	        car_list[i].append(car)
	        Carnum = Carnum + 1
	        newcarindex[i] = newcarindex[i] + 1
	        if random.random() * 100 > Percent:
